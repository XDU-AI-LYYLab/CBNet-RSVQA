...
class Fusion(nn.Module):
    def __init__(self, in_dim=768):
        super().__init__()
        self.w1  = nn.Linear(2 * in_dim, in_dim)
        self.elu = nn.ELU()
        self.dropout = nn.Dropout(0.2)
    ...
    def forward(self, v1, v3):
        v1 = v1.mean(dim=1)
        v3 = v3.mean(dim=1)
        h1 = torch.cat([v1, v3], dim=1)
        h1 = self.elu(self.w1(h1)) # -> [B, D]
        h = self.dropout(h1)
        return h
...
class ManualLRStepCallback(pl.Callback):
    """
    一个完全手动的、按 Epoch 分阶段设置学习率的回调。
    """
    def __init__(self, base_lr=1e-5):
        ...
        # ✨ 在这里定义您想要的完整学习率计划
        # 格式：(epoch_start, epoch_end, learning_rate)
        # 注意：epoch 是从 0 开始计数的
        self.lr_schedule = [
            # 阶段 1: Warmup
            (0, 0, self.base_lr * 1.0), # Epoch 0: 1e-5
            (1, 1, self.base_lr * 2.0), # Epoch 1: 2e-5
            (2, 3, self.base_lr * 1.0), # Epoch 2: 1e-5
            
            # 阶段 2: Cooldown
            (4, 5, self.base_lr * 0.2), # Epoch 3: 1e-5
            
            # 阶段 3: 第一个衰减点 (假设您在第10轮后衰减)
            (6, 7, self.base_lr * 0.2*0.2), # Epoch 10-19: 2e-6
            
            # 阶段 4: 第二个衰减点 (假设您在第20轮后衰减)
            (8, 9, self.base_lr * 0.01), # Epoch 20+: 2e-7
            (10, 50, self.base_lr * 0.001)
        ]
        
        
        # 为了提高效率，将计划转换为字典
        self.lr_map = {}
        for (start, end, lr) in self.lr_schedule:
            for epoch in range(start, end + 1):
                self.lr_map[epoch] = lr
    ...
    def on_train_epoch_start(self, trainer, pl_module):
        ...
        # 查找当前 epoch 对应的学习率
        # 如果在 map 中没找到，就使用上一个 epoch 的学习率
        # 或者默认使用 base_lr
        new_lr = self.lr_map.get(epoch, None)
        
        if new_lr is not None:
            # 如果在我们的计划表中找到了这一轮的 LR，就设置它
            for param_group in optimizer.param_groups:
                param_group['lr'] = new_lr
            print(f"\n【手动调度器】 Epoch {epoch}: 学习率设置为 {new_lr:.8f}")
...
class VQAModel(pl.LightningModule):
    ...
    def on_validation_epoch_end(self):
        ...
        if OA > self.best_valid_OA:
            ...
            torch.save(self.state_dict(), "best_model_full0.ckpt")
        ...
        self.print(
            ...
            f"Model Path: best_model_full0.ckpt"
        )
    ...
    def on_fit_end(self):
        ...
        # 创建保存图片的目录
        plot_dir = "loss_plots_full"
        if not os.path.exists(plot_dir):
            os.makedirs(plot_dir)

        # 保存图片到指定目录
        plot_path = os.path.join(plot_dir, "loss_curve_full0.png")
